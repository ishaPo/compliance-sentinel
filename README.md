...
1. **Gemini 2.0 Flash**, called directly, using its native structured-output mode (a JSON schema, not prompt-and-hope). Free tier, no card required.
2. If that call fails, is rate-limited, or the key is missing — **OpenRouter**, called with an ordered `models` array of free models. OpenRouter automatically tries the next model in the list if one is unavailable; this is its own documented fallback mechanism, not hand-rolled retry logic.
3. If both are unreachable, the run still completes: sections keep their heuristic classification, severity defaults conservatively, and the interpretation reads "No interpretation available" rather than the run failing outright. Every step of this chain is visible in the agent trail, including which model actually answered.
